...
def planetary_eq():
	...
	rtGM = np.sqrt(G * (m_star+m_planet))
...
def equations_signal(R, V, times, t0, a_earth, m_star, m_planet, a_AU, tau, e, omega, Omega, inc, G=4*np.pi*np.pi):
	# finding initial values of d_bc, delta_bc and alpha_bc 
	d, delta, alpha = simple_func(R)
	...
	planetary_term_dec = (-cosd*DELTA_Z + sind*(DELTA_X*cosa+DELTA_Y*sina))
	planetary_term_ra  = (1/cosd) * (sina*DELTA_X-cosa*DELTA_Y) 

	return (pm_term_dec, pm_term_ra, parallax_dec, parallax_ra, planetary_term_dec, planetary_term_ra)
