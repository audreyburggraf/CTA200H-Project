...
plt.scatter(SN_1P, P_orb_plot_1P, color=colors[0], label="$P_{orb}$"+str(P_orb_array[0]))
...
plt.scatter(SN_1P, -1*Delta_BIC_1P, color=colors[0], label="$P_{orb}$"+str(P_orb_array[0]))
plt.xscale('log')
plt.yscale('log')
...
for k in range(K):
    plt.scatter(a_hat_KP[k]/sigma_err_KP[k], P_orb_plot_KP[k], color=colors[k], label="$P_{orb}=$"+str(P_orb_array[k]))
...
for k in range(K):
    plt.scatter(a_hat_KP[k]/sigma_err_KP[k], -1*Delta_BIC_KP[k], label="$P_{orb}=$"+str(P_orb_array[k]), color=colors[k])
...
for k in range(K):
    for j in range(J):       
        ...
        plt.scatter(a_hat_JKP[j,k]/sigma_err_JKP[j,k], P_orb_plot_KP[k], label= "$P_{orb}=$"+str(P_orb_array[k]), color=colors[2*k+j], s=marker_size)
...
for k in range(K):
    for j in range(J):
        plt.scatter(a_hat_JKP[j,k]/sigma_err_JKP[j,k], -1*Delta_BIC_JKP[j,k], label= "$P_{orb}=$"+str(P_orb_array[k]), color=colors[2*k+j])
